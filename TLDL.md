...
- 取得品質の参考診断（point format、return、scan angle、GPS time の availability / sampled coverage）
- 表示点ベースの地表候補アシスト（class 2 ground 優先、薄い場合は下位パーセンタイル推定）
- 手動コピー型診断レポートと作業メモ出力（自動送信なし）
...
- 測量成果DTM、設計面、切盛根拠、納品用地形モデルの生成
- 勾配、土量、2時期差分、ROI保存、視点保存、workspace import/export
- Safari/Firefox 対応
...
### High-6: 地表候補アシストのDTM誤認

- 根拠: 地表候補は表示点ベースで、class 2 ground または p05 推定を使う参考診断
- 誤解パターン: 「地表候補=成果品DTM」「勾配・土量に使える」と解釈
- 実害: 解析成果や施工判断の根拠として過信される
- 検知方法: 統計パネルの方式、confidence、有効セル率、ドキュメント説明を確認
- 抑止策: UI/README/FAQ/運用手順で「成果品DTMではない」を明示
- 優先度: High

### Medium-3: 取得品質診断の精度保証誤認

- 根拠: 取得品質は属性 availability と sampled coverage の参考診断
- 誤解パターン: 「score が高い=測量精度が保証された」と解釈
- 実害: 機器精度、標定、点検記録の確認を省略する
- 検知方法: 手動診断JSONの `acquisitionQuality` と warnings を確認
- 抑止策: 成果品適合性や測量精度は別資料で確認する運用に固定
- 優先度: Medium
