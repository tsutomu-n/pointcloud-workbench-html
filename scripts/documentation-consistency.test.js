...
test("CRS diagnostics docs keep local-only and no-conversion boundaries aligned", () => {
  ...
  expect(readme).toContain("X/Y入れ替え候補");
  ...
  expect(readmeEn).toContain("swapped-X/Y candidate");
  ...
  expect(ops).toContain("location.latLon.alternateAxis");
  ...
  expect(ref).toContain("Subresource Integrity");
