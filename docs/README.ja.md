...
ブラウザで動く、LAS/LAZ 対応の単一 HTML 点群ワークベンチです。3D/2D 表示、スライス、分類、統計確認、取得品質確認、地表候補アシストに対応します。
...
- ローカル LAS/LAZ の header / VLR / EVLR から WKT、GeoTIFF、EPSG 候補を読む CRS 診断
- LAS/LAZ の point format、return、scan angle、GPS time を使った取得品質の参考診断
- class 2 ground または下位パーセンタイル推定による、表示点ベースの地表候補アシスト
- 表示、断面、計測、異常候補、取得品質、地表候補の要約をコピーする作業メモコピー
...
4. 読み込み完了後、視点切替、色分け、距離計測、スライス、自動分類、統計表示を必要に応じて使います。
5. 統計パネルで CRS 診断、地表候補、地表方式を確認し、必要なら「診断情報をコピー」または「作業メモコピー」を使います。
...
- 取得品質は利用可能な LAS/LAZ 属性の有無と sampled coverage の参考診断です。測量精度、計測機器品質、成果品適合性を保証しません。
- 地表候補アシストは表示点ベースの参考診断です。測量成果DTM、設計面、切盛根拠、納品用地形モデルとしては扱わないでください。
- 作業メモコピーは手動コピーのみです。ローカルファイル名、点群 payload、元LAS座標配列は含めません。
...
- 距離計測は表示中のサンプリング点への計測です。元LAS/LAZの全点最近傍探索ではありません。計測結果はページ内の一時履歴として最大20件まで残せます。距離は `m相当` として表示し、元データの座標単位に依存します。読み込みプロファイルの推定ピークRAMには、計測用に保持する元座標メモリも含まれます。
